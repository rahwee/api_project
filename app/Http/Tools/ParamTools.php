...
class ParamTools {
    ...
    */
    public static function getNameDBByGid($gid)
    {
        ...
        return $prefix.str_replace("-",'_', $gid);
    }
    ...
    */
    ...
    public static function syncData(string $table, $data, string $database) : void
    {
        ...
        $log = "Synchronized [".$table."] from [".$oldConnection."] to [".$database."]";
        logReporter(Constants::SOURCE_LOG, $log, [
            'response' => $ret,
            'data'     => count($ret['errors']??[]) > 0 ? $dataSync : []
        ], 'info');
